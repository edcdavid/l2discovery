...
func MatchingOptionalSelectorPTP(toFilter []ptpv1.NodePtpDevice) ([]ptpv1.NodePtpDevice, error) {
	...
	toMatch, err := l2client.Client.K8sClient.CoreV1().Nodes().List(context.Background(), metav1.ListOptions{
	...
	for index1 := range toFilter {
		for index2 := range toMatch.Items {
			if toFilter[index1].Name == toMatch.Items[index2].Name {
				res = append(res, toFilter[index1])
				break
			}
		}
	}
